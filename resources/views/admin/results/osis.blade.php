...
@section('title', 'Real Count Hasil OSIS')

@push('styles')
<style>
    /* Header dan Indikator */
    .page-header { background: linear-gradient(45deg, #4e73df, #224abe); color: white; border-radius: 12px; padding: 1.5rem; margin-bottom: 2rem; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
    ...
    .live-indicator .dot { width: 8px; height: 8px; background-color: #1cc88a; border-radius: 50%; margin-right: 0.5rem; animation: pulse 1.5s infinite; }
    ...
    /* [BARU] Aturan untuk menyembunyikan sidebar saat fullscreen */
...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h2 class="mb-0 fw-bold">Real Count Pemilihan OSIS</h2>
        <p class="mb-0 opacity-75">Total Suara Masuk: <span id="total-votes-header">{{ $totalOsisVotes }}</span></p>
    </div>
    <div>
...
<div class="card">
    ...
    <div class="card-body">
        ...
        @if(count($osisCandidates) === 0)
             <div class="empty-state">
                <i class="bi bi-people-fill fs-1 text-muted"></i>
                <h5 class="mt-3">Belum Ada Kandidat OSIS</h5>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const colorPalette = ['#4e73df', '#1cc88a', '#f6c23e', '#36b9cc', '#e74a3b', '#858796'];
    ...
    function createOrUpdateChart(candidates, totalVotes) {
        ...
        if (!candidates || candidates.length === 0) {
            container.innerHTML = `<div class="empty-state"><i class="bi bi-people-fill fs-1 text-muted"></i><h5 class="mt-3">Belum Ada Kandidat OSIS</h5></div>`;
            return;
        ...
        voteChartInstance = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: candidates.map(c => `${c.name_ketua} & ${c.name_wakil}`),
                datasets: [{
                    label: 'Jumlah Suara',
    ...
    async function fetchData() {
        try {
            ...
            document.getElementById('total-votes-header').innerText = data.totalOsisVotes.toLocaleString('id-ID');
            createOrUpdateChart(data.osisCandidates, data.totalOsisVotes);
        } catch (error) {
